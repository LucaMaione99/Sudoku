<template>
	<Transition name="fade">
		<div v-if="storeModal.activeModalConfirm" class="bg-black/20 w-screen h-screen fixed top-0 z-50">
			<div class="flex justify-center items-center h-screen">
				<div class="relative bg-white min-h-[300px] min-w-[350px] md:min-w-[550px] md:max-w-[800px] rounded-lg shadow">
					<img
						...
						alt="" />
					<div class="absolute w-full h-full z-0" v-html="storeModal.backgroundHtml"></div>
					<div class="z-[99999] relative">
						<button
							@click="storeModal.closeModal"
							type="button"
							class="absolute top-3 right-2.5 text-platform-gray bg-transparent hover:bg-platform-light-gray hover:text-platform-text rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
						...
						<div class="p-6 flex flex-col justify-between text-center min-h-[300px] gap-y-4">
							<div>
								...
								<h3 v-if="storeModal.title" class="text-2xl font-semibold whitespace-pre-line">
									{{ storeModal.title }}
								</h3>
							</div>
							<h3
								v-if="storeModal.msg"
								class="mb-5 font-normal text-platform-gray whitespace-pre-line"
								v-html="storeModal.msg"></h3>
							<div v-if="storeModal.contentHtml" v-html="storeModal.contentHtml"></div>
							<div class="flex justify-end space-x-4">
								<SecondaryButton @click="storeModal.closeModal">{{
									storeModal.buttonCancelText
								}}</SecondaryButton>
								<PrimaryButton @click="storeModal.confirmModal">{{
									storeModal.buttonConfirmText
								}}</PrimaryButton>
							</div>
						</div>
					</div>
...
<script setup>
import { useModalStore } from "@/stores/handleModal";
import PrimaryButton from "../../core/buttons/PrimaryButton.vue";
import SecondaryButton from "../../core/buttons/SecondaryButton.vue";
